...
function appointmentScopeCondition(group, appointmentAlias = "a") {
  if (group === "MANAGER") {
    return `AND (@BranchId IS NULL OR ${appointmentAlias}.BranchId = @BranchId)`;
  }
  if (group === "STAFF") {
    return `AND (@EmployeeId IS NULL OR ${appointmentAlias}.EmployeeId = @EmployeeId)`;
  }
  return "";
}
...
async function personalActivity(context) {
  ...
  if (context.scope.employeeId) {
    const apptResult = await context.request.query(`
      SELECT 
        CASE Status
          WHEN 'COMPLETED' THEN N'Hoàn thành ca dịch vụ'
          WHEN 'CONFIRMED' THEN N'Xác nhận ca hẹn'
          WHEN 'CHECKED_IN' THEN N'Check-in tiếp nhận khách'
          WHEN 'IN_PROGRESS' THEN N'Đang thực hiện dịch vụ'
          WHEN 'CANCELLED' THEN N'Hủy / Đổi lịch hẹn'
          ELSE N'Thao tác ca hẹn khác'
        END AS Label,
        COUNT(*) AS Value
      FROM Appointments
    ...
    if (apptResult.recordset.length > 0) {
      return mapSingleSeries(apptResult.recordset);
    }
  }

  const generalResult = await context.request.query(`
    SELECT TOP 5 ISNULL(ActionName, N'Thao tác hệ thống') AS Label, COUNT(*) AS Value
    FROM SystemLogs
    WHERE CreatedAt >= @StartDate
      AND CreatedAt < DATEADD(DAY, 1, @EndDate)
    GROUP BY ActionName
    ORDER BY Value DESC
  `);
  return mapSingleSeries(generalResult.recordset);
}
